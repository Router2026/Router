...
export function buildGenerateTripPrompt(input: TripInput, pois: Poi[]): string {
  ...
  const DIFFICULTY_GUIDE: Record<string, string> = {
    ...
    moderate: "Mix of light activities and moderate hikes (2–5 km). Balance physical activity with relaxed stops.",
    challenging: "Include longer hikes (5–10 km) and physically demanding trails. Fewer rest stops, more active experiences.",
    extreme: "Prioritize the most strenuous multi-hour hikes and trails (10+ km). Maximize physical challenge; minimize passive stops.",
  };
  ...
  const AVG_DURATION: Record<string, number> = {
    restaurant: 75, coffee_trail: 30, hiking_trail: 120, attraction: 90, campsite: 480,
  };
  const hasCampsite = input.poiCategories.includes("campsite");
  const nonCampCategories = input.poiCategories.filter((c) => c !== "campsite");
  const avgDuration = nonCampCategories.length > 0
    ? nonCampCategories.reduce((sum, c) => sum + (AVG_DURATION[c] ?? 75), 0) / nonCampCategories.length
    : 75;
...
export function buildRegenerateStopPrompt(
  ...
  if (stopIndex < 0 || stopIndex >= day.stops.length) {
    throw new Error(`Invalid stopIndex ${stopIndex} — day ${dayIndex + 1} has ${day.stops.length} stops`);
  }
