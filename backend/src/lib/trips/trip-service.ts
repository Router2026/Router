...
const PLACEHOLDER_NAMES = new Set(['place name exactly as listed', 'place name']);
...
export async function generateTrip(
  ...
  const pois =
    poisOverride ?? (await getRelevantPois(input.poiCategories, input.areas, input.subAreas));
  if (pois.length === 0) {
...
export async function regenerateStop(
  ...
  if (dayIndex >= days.length) {
    throw Object.assign(new Error(`Day ${dayIndex + 1} does not exist in this trip`), {
      status: 400,
    });
  }
  if (stopIndex >= days[dayIndex].stops.length) {
    throw Object.assign(new Error(`Stop ${stopIndex + 1} does not exist on day ${dayIndex + 1}`), {
      status: 400,
    });
  }
  ...
  await db.update(trips).set({ generatedPlan: updatedPlan }).where(eq(trips.uuid, uuid));

  return { plan: updatedPlan, stop: newStop };
}

export async function getTripByUuid(uuid: string): Promise<TripRecord | null> {
  const results = await db.select().from(trips).where(eq(trips.uuid, uuid)).limit(1);
