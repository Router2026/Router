...
function missingCategories(plan: TripPlan, required: string[]): string[] {
  const present = new Set(
    plan.days.flatMap((d) =>
      d.stops.map((s) => s.category.split(",")[0].trim()),
    ),
  );
...
export async function generateTrip(
  input: TripInput,
  poisOverride?: Poi[],
): Promise<{ uuid: string; plan: TripPlan }> {
  const pois =
    poisOverride ??
    (await getRelevantPois(input.poiCategories, input.areas, input.subAreas));
  if (pois.length === 0) {
  ...
  if (missing.length > 0) {
    console.warn(
      `[generateTrip] Missing categories (not retrying): ${missing.join(", ")}`,
    );
  }
...
export async function regenerateStop(
  ...
  if (dayIndex >= days.length) {
    throw Object.assign(
      new Error(`Day ${dayIndex + 1} does not exist in this trip`),
      {
        status: 400,
      },
    );
  ...
  if (stopIndex >= days[dayIndex].stops.length) {
    throw Object.assign(
      new Error(`Stop ${stopIndex + 1} does not exist on day ${dayIndex + 1}`),
      {
        status: 400,
      },
    );
  ...
  const pois = await getRelevantPois(
    trip.poiCategories,
    trip.poiAreas,
    trip.poiSubAreas,
  );
  const llm = getLLMProvider();
  const newStop = await llm.regenerateStop(
    trip.generatedPlan,
    dayIndex,
    stopIndex,
    pois,
...
export async function getTripByUuid(uuid: string): Promise<TripRecord | null> {
  ...
  if (!lenient.success) {
    console.error(
      "[getTripByUuid] Stored plan is unreadable:",
      lenient.error.message,
    );
    return null;
  }
  ...
  if (recovered.days.length === 0) {
    console.error(
      "[getTripByUuid] Stored plan has no valid stops after sanitization",
    );
    return null;
  }
