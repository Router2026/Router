...
import { getLocations, getLocationById, getLocationsInBounds, upsertLocation } from './location-service'
...
describe('upsertLocation', () => {
  it('calls insert with ON CONFLICT', async () => {
    ...
    const id = await upsertLocation({
      name: 'New Place', latitude: 32.0, longitude: 35.0, source: 'manual',
    })
    expect(id).toBe(10)
