...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(errorResponse('Failed to fetch reviews', 'DB_ERROR'), { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
      ...
      if (sbUser?.email) {
        ...
        if (rows.length) {
          ...
          resolvedReviewerName =
            (rows[0].full_name as string) || (rows[0].username as string) || resolvedReviewerName;
        }
      } else {
        // Fallback: legacy JWT
    ...
    return NextResponse.json(errorResponse('Failed to create review', 'DB_ERROR'), { status: 500 });
  }
}
