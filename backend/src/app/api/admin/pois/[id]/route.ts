...
export async function PUT(
  ...
  try {
    ...
    const [updated] = await db
      ...
      .returning();
    if (!updated) {
      return NextResponse.json(errorResponse("POI not found", "NOT_FOUND"), {
        status: 404,
      });
    }
    ...
    return NextResponse.json(
      errorResponse("Failed to update POI", "SERVER_ERROR"),
      {
        status: 500,
      },
    );
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      errorResponse("Failed to delete POI", "SERVER_ERROR"),
      {
        status: 500,
      },
    );
