...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (String(admin.id) === id) {
    return NextResponse.json(errorResponse('Cannot delete your own account', 'FORBIDDEN'), {
      status: 403,
    });
  }
  try {
    ...
    return NextResponse.json(errorResponse('Failed to delete user', 'DB_ERROR'), { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (sets.length === 0) {
      return NextResponse.json(errorResponse('No valid fields to update', 'VALIDATION_ERROR'), {
        status: 400,
      });
    }
    vals.push(id);
    ...
    return NextResponse.json(errorResponse('Failed to update user', 'DB_ERROR'), { status: 500 });
  }
}
