...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!poi) {
      return NextResponse.json(errorResponse('Community POI not found', 'NOT_FOUND'), {
        status: 404,
      });
    }
    ...
    return NextResponse.json(errorResponse('Failed to fetch community POI', 'DB_ERROR'), {
      status: 500,
    });
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(errorResponse('Failed to update community POI', 'DB_ERROR'), {
      status: 500,
    });
  }
}
