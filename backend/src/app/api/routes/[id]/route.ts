...
export async function GET(
  ...
  try {
    ...
    if (!route) {
      return NextResponse.json(errorResponse("Route not found", "NOT_FOUND"), {
        status: 404,
      });
    }
    ...
    return NextResponse.json(
      errorResponse("Failed to fetch route", "DB_ERROR"),
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      errorResponse("Failed to delete route", "DB_ERROR"),
      { status: 500 },
    );
  }
}
