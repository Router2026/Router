  ...
  try {
    ...
    if (!access_token || !refresh_token || !password)
      return NextResponse.json(
        errorResponse("Tokens and password required", "VALIDATION_ERROR"),
        {
          status: 400,
        },
      );
    ...
    if (sessionError)
      return NextResponse.json(
        errorResponse("Invalid or expired reset link", "INVALID_TOKEN"),
        {
          status: 400,
        },
      );
    ...
    if (error)
      return NextResponse.json(
        errorResponse("Failed to reset password", "AUTH_ERROR"),
        {
          status: 400,
        },
      );
    ...
    return NextResponse.json(
      errorResponse("Failed to reset password", "SERVER_ERROR"),
      {
        status: 500,
      },
    );
