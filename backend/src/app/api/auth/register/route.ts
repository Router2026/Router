...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing.length > 0)
      return NextResponse.json(
        errorResponse("Email or username already in use", "CONFLICT"),
        {
          status: 409,
        },
      );
    ...
    if (error) {
      ...
      return NextResponse.json(errorResponse(error.message, "AUTH_ERROR"), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(
      successResponse({ user: rows[0], requiresVerification: true }),
      {
        status: 201,
      },
    );
    ...
    return NextResponse.json(errorResponse("Registration failed", "DB_ERROR"), {
      status: 500,
    });
  }
}
