  ...
  try {
    ...
    if (!error && data.session) {
      if (!data.user.email_confirmed_at)
        return NextResponse.json(
          errorResponse('Please verify your email before logging in', 'EMAIL_NOT_VERIFIED'),
          { status: 403 }
        );
      ...
      if (!rows.length) {
        ...
        const result = await rawDb.query(
          ...
          [
            normalizedEmail,
            meta.full_name || normalizedEmail,
            meta.username || normalizedEmail.split('@')[0],
          ]
        );
        rows = result.rows;
      }
      return NextResponse.json(
        successResponse({ user: rows[0], token: data.session.access_token })
      );
    }
    ...
    if (!user || !user.password_hash)
      return NextResponse.json(errorResponse('Invalid email or password', 'AUTH_ERROR'), {
        status: 401,
      });
    ...
    if (signUpData?.session) {
      return NextResponse.json(
        successResponse({ user: safeUser, token: signUpData.session.access_token })
      );
    }

    // signUp created the account but needs email confirmation — try signing in again
    const { data: retried } = await supabase.auth.signInWithPassword({
      email: normalizedEmail,
      password,
    });
    if (retried?.session) {
      return NextResponse.json(
        successResponse({ user: safeUser, token: retried.session.access_token })
      );
    }
