...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!token || typeof token !== 'string') {
      return NextResponse.json(errorResponse('token is required', 'VALIDATION_ERROR'), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(errorResponse('Failed to register push token', 'DB_ERROR'), {
      status: 500,
    });
  }
}
