...
async function resolveUserId(req: NextRequest): Promise<number | null> {
  ...
  if (sbUser?.email) {
    const { rows } = await rawDb.query(`SELECT id FROM users WHERE email = $1 LIMIT 1`, [
      sbUser.email,
    ]);
...
export async function GET() {
  try {
    ...
    return NextResponse.json(errorResponse('Failed to fetch community POIs', 'DB_ERROR'), {
      status: 500,
    });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      return NextResponse.json(errorResponse('Authentication required', 'AUTH_ERROR'), {
        status: 401,
      });
    }
    ...
    return NextResponse.json(errorResponse('Failed to create community POI', 'DB_ERROR'), {
      status: 500,
    });
  }
}
