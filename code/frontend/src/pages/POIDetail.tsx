...
import { base44, ALL_POIS, type POI } from '../api';
...
export default function POIDetail() {
  ...
  useEffect(() => {
    // Find by id, fallback to first
    const found = ALL_POIS.find(p => p.id === poiId) || ALL_POIS[0];
    setPoi(found);
    base44.entities.Review.filter().then(setReviews);
    base44.entities.CommunityReport.filter().then(setReports);
  }, [poiId]);
  ...
  return (
    <div style={{ background: '#f0f4f3', minHeight: '100vh', width: '100%' }}>
      ...
      <div style={{
        position: 'relative', height: 350, width: '100%',
        backgroundImage: `url(${poi.main_image})`,
        ...
        {/* Centered Content Wrapper for Hero */}
        <div style={{ maxWidth: 600, margin: '0 auto', height: '100%', position: 'relative' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {/* Share */}
              <button style={{
                background: 'rgba(255,255,255,0.88)', border: 'none', borderRadius: 14,
                width: 42, height: 42, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                transition: 'transform 0.2s',
              }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1a2e2a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <button onClick={() => setIsFav(!isFav)} style={{
                ...
                <svg width="18" height="18" viewBox="0 0 24 24"
                  ...
                  stroke={isFav ? '#ef4444' : '#1a2e2a'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button onClick={() => navigate(-1)} style={{
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1a2e2a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{
            ...
            padding: '20px 20px 32px', direction: 'rtl', zIndex: 10
          }}>
            <span style={{
              display: 'inline-block', background: diffColor, color: '#fff',
              borderRadius: 8, padding: '4px 12px', fontSize: 12, fontWeight: 800, marginBottom: 12,
              boxShadow: '0 2px 8px rgba(0,0,0,0.2)'
            }}>{poi.difficulty}</span>
            <h1 style={{ fontSize: 32, fontWeight: 900, color: '#fff', marginBottom: 8, textShadow: '0 2px 12px rgba(0,0,0,0.5)' }}>
              {poi.name}
            </h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.95)' }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 500, textShadow: '0 1px 4px rgba(0,0,0,0.5)' }}>{poi.region}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 16px', marginTop: -20, position: 'relative', zIndex: 20, paddingBottom: 100 }}>
        ...
        <div style={{
          ...
          boxShadow: '0 8px 32px rgba(0,0,0,0.08)', padding: '24px 20px', marginBottom: 16,
        }}>
          {/* Rating + Category */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, direction: 'rtl' }}>
            ...
            <span style={{
              background: '#f0fdf8', color: '#0d9e6e', borderRadius: 10,
              padding: '6px 14px', fontSize: 13, fontWeight: 800,
            }}>{poi.category}</span>
          </div>

          {/* Description */}
          <p style={{ fontSize: 15, color: '#475569', lineHeight: 1.8, textAlign: 'right', marginBottom: 24 }}>
            {poi.description}
          </p>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end', direction: 'rtl' }}>
            ...
            {poi.has_water && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#eff6ff', borderRadius: 12, padding: '8px 14px', fontSize: 13, fontWeight: 700, color: '#0284c7' }}>
                💧 יש מים
              </span>
            )}
            {poi.has_shade && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#f0fdf4', borderRadius: 12, padding: '8px 14px', fontSize: 13, fontWeight: 700, color: '#16a34a' }}>
                🌿 יש צל
              </span>
            )}
            {poi.accessible && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#faf5ff', borderRadius: 12, padding: '8px 14px', fontSize: 13, fontWeight: 700, color: '#7c3aed' }}>
                ♿ נגיש
              </span>
            )}
          </div>
        ...
        <div style={{
          ...
          boxShadow: '0 4px 16px rgba(0,0,0,0.04)', marginBottom: 16, display: 'flex', direction: 'rtl'
        }}>
          {[
        ...
        {tab === 'reports' && (
          <div style={{ direction: 'rtl' }}>
            {reports.slice(0, 3).map(r => (
              <div key={r.id} style={{ background: '#fff', borderRadius: 18, padding: '16px', marginBottom: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>לפני זמן קצר</span>
            ...
            <button onClick={() => navigate(`/AddReport?poi_name=${poi.name}`)} style={{
              width: '100%', padding: '16px', border: '2px dashed #0d9e6e',
              borderRadius: 18, background: '#f0fdf8', color: '#0d9e6e',
              fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'Heebo, sans-serif', marginBottom: 16,
              transition: 'all 0.2s ease'
            }}>+ הוסף דיווח</button>
          </div>
        )}
        ...
        {tab === 'reviews' && (
          <div style={{ direction: 'rtl' }}>
            ...
            <button onClick={() => navigate(`/AddReview?poi_name=${poi.name}`)} style={{
              width: '100%', padding: '16px', border: '2px dashed #0d9e6e',
              borderRadius: 18, background: '#f0fdf8', color: '#0d9e6e',
              fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'Heebo, sans-serif', marginBottom: 16,
              transition: 'all 0.2s ease'
        ...
        {/* Navigate CTA */}
        <button onClick={() => window.open(`https://waze.com/ul?q=${encodeURIComponent(poi.name)}`, '_blank')} style={{
