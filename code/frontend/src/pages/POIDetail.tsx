...
// ── Favorite Heart Button (Feature 3) ────────────────────────────────────────
function FavoriteButton({ poiId, isLoggedIn }: { poiId: string; isLoggedIn: boolean }) {
  ...
  useEffect(() => {
    ...
    api.favorites.list()
      .then(ids => {
        setIsFav(ids.includes(Number(poiId)));
        setReady(true);
      })
      .catch(() => setReady(true));
  }, [poiId, isLoggedIn]);
  ...
  const toggle = async () => {
    ...
    try {
      if (isFav) {
        await api.favorites.remove(Number(poiId));
        setIsFav(false);
      } else {
        await api.favorites.add(Number(poiId));
        setIsFav(true);
      }
    } catch {
      // silently ignore network error — state unchanged
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      style={{
        background: isFav ? 'rgba(239,68,68,0.18)' : 'rgba(255,255,255,0.88)',
    ...
    >
      <svg
        ...
        stroke={isFav ? '#ef4444' : '#1a2e2a'}
        strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
        style={{ transition: 'transform 0.2s ease', transform: isFav ? 'scale(1.2)' : 'scale(1)' }}
      >
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', height: '100%', position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', top: 16, left: 16, right: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 12 }}>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              {/* Feature 3: real backend-linked favorite */}
              <FavoriteButton poiId={poiId} isLoggedIn={isLoggedIn} />
            </div>
            <button
              onClick={() => navigate(-1)}
              style={{ background: 'rgba(255,255,255,0.88)', border: 'none', borderRadius: 14, width: 42, height: 42, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
