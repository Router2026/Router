...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    base44.entities.UserProfile.filter().then(d => { if (d.length > 0) setProfile(d[0]); });
    base44.entities.Trip.filter().then(setTrips);
  }, []);

  if (!user) return <div style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>טוען...</div>;
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', width: '100%', direction: 'rtl' }}>
      ...
      <div style={{
        background: '#fff', borderBottom: '1px solid #f0f0f0', width: '100%'
      }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {user.full_name?.charAt(0) || 'ע'}
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 20, fontWeight: 900, color: '#1a2e2a' }}>{user.full_name}</div>
