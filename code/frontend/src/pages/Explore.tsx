...
function FilterPanel({
  ...
  const totalFilters = selectedRegions.length + selectedCategories.length + selectedDifficulties.length
    ...
    <div style={{
      ...
      <div style={{
        ...
        <FilterSection title="רמת קושי">
          <TagGrid items={DIFFICULTIES} selected={selectedDifficulties}
        ...
        <FilterSection title="מאפיינים">
          {[
            ...
            <label key={f.label} style={{
              ...
              <div onClick={() => f.set(!f.state)} style={{
...
function POICard({ poi, onFav, favs }: { poi: POI; onFav: (id: string) => void; favs: Set<string> }) {
  ...
  return (
    <div onClick={() => navigate(`/POIDetail?id=${poi.id}`)} style={{
      ...
      <div style={{ position: 'relative', height: 160 }}>
        ...
        <span style={{
          ...
          background: '#fff', color: diff.color,
      ...
      <div style={{ padding: '14px 14px 16px', direction: 'rtl' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 10, justifyContent: 'flex-end', alignItems: 'center' }}>
          ...
          {poi.has_shade && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 3, color: '#16a34a', fontSize: 11, fontWeight: 600 }}>🌿 צל</div>
          )}
        </div>
      </div>
    </div>
...
export default function Explore() {
  ...
  const removeRegion = (r: string) => setSelRegions(prev => prev.filter(x => x !== r));
