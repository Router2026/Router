...
const makePoi = (id: string): POI => ({
  id, name: `POI ${id}`, description: '', category: 'טבע',
  region: 'north', latitude: 32.0, longitude: 35.0,
  images: [], main_image: '', difficulty: 'בינוני', average_rating: 4.0,
})
...
describe('TripBucketContext', () => {
  ...
  it('addPoi adds an item and updates count', () => {
    ...
    const { getByTestId } = wrap(ctx => { captured = ctx })

    act(() => { captured.addPoi(makePoi('1')) })
  ...
  it('removePoi removes an item', () => {
    ...
    act(() => { captured.addPoi(makePoi('1')) })
    act(() => { captured.removePoi('1') })
    expect(getByTestId('count').textContent).toBe('0')
  })

  it('hasPoi returns false for unknown id', () => {
    let captured: ReturnType<typeof useTripBucket>
    wrap(ctx => { captured = ctx })
    act(() => {})
