...
describe('api auth', () => {
  it('auth.login returns user and token', async () => {
    mockFetch.mockResolvedValue(mockOk({
      data: { user: { id: 1, username: 'alice', xp_points: 0, level: 'מתחיל' }, token: 'abc' },
    }))
  ...
  it('auth.login throws ApiError on 401', async () => {
    mockFetch.mockResolvedValue(mockError(401, { error: { message: 'Invalid credentials' } }))
    await expect(api.auth.login('x@x.com', 'wrong')).rejects.toThrow('Invalid credentials')
  })
...
describe('api locations', () => {
  it('list returns mapped POIs', async () => {
    mockFetch.mockResolvedValue(mockOk({
      data: [
        { id: 1, name: 'test', latitude: '32.0', longitude: '35.0', images: [], average_rating: '4.5' },
      ],
    }))
  ...
  it('get fetches single location by id', async () => {
    mockFetch.mockResolvedValue(mockOk({
      data: { id: 5, name: 'Ein Gedi', latitude: '31.0', longitude: '35.3', images: [], average_rating: '4.8' },
    }))
...
describe('api trips (routes)', () => {
  it('list returns mapped trips', async () => {
    mockFetch.mockResolvedValue(mockOk({
      data: [{ id: 1, name: 'Route 1', total_duration_hours: '3', stops: [] }],
    }))
  ...
  it('delete calls DELETE /routes/:id', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/routes/42', expect.objectContaining({ method: 'DELETE' }))
  })
})
