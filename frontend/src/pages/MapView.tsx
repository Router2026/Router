    ...
    if (shouldCluster) {
      ...
      groups.forEach(g => {
        if (g.pois.length === 1) {
          ...
          const m = L.marker([g.lat, g.lng], { icon: makeClusterIcon(g.pois.length, color) });
          m.on('click', (e) => {
    ...
    <div ref={containerRef} style={{ position: 'absolute', top: 60, left: 10, right: 10, zIndex: 1500, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.18)', padding: '20px', direction: 'rtl', maxHeight: '60vh', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {categories.map(c => {
            ...
            return <button key={c} onClick={() => toggle(selCats, setSelCats, c)} style={{ padding: '5px 10px', borderRadius: 16, border: `2px solid ${a ? '#0d9e6e' : '#e2e8f0'}`, background: a ? '#0d9e6e' : '#fff', color: a ? '#fff' : '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{c}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {DIFFICULTIES.map(d => {
            ...
            return <button key={d} onClick={() => toggle(selDiffs, setSelDiffs, d)} style={{ padding: '5px 10px', borderRadius: 16, border: `2px solid ${a ? '#0d9e6e' : '#e2e8f0'}`, background: a ? '#0d9e6e' : '#fff', color: a ? '#fff' : '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{d}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {[{ label: '💧 מים', state: hasWater, set: setHasWater }, { label: '🌿 צל', state: hasShade, set: setHasShade }, { label: '♿ נגיש', state: accessible, set: setAccessible }].map(f => (
          <button key={f.label} onClick={() => f.set(!f.state)} style={{ padding: '6px 12px', borderRadius: 16, border: `2px solid ${f.state ? '#0d9e6e' : '#e2e8f0'}`, background: f.state ? '#0d9e6e' : '#fff', color: f.state ? '#fff' : '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{f.label}</button>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 72px)', overflow: 'hidden', direction: 'rtl' }}>
        ...
        {!error && selectedRegion ? (
          <>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 1 }}>{loading ? 'טוען...' : `${pois.length} אתרים`}{activeFilterCount > 0 ? ` (${activeFilterCount} פילטרים)` : ''}</div>
            </div>
            ...
            <button onClick={() => setSelectedRegion(null)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 10, padding: '6px 12px', cursor: 'pointer', fontSize: 13, color: '#64748b', fontFamily: 'Heebo, sans-serif', fontWeight: 600 }}>ביטול אזור</button>
          </>
        ) : !error && (
      ...
      <OverlayFilter
        ...
        selDiffs={selDiffs} setSelDiffs={setSelDiffs}
      ...
      <MapContainer center={[31.5, 35.0]} zoom={8} style={{ width: '100%', height: '100%', zIndex: 1 }} zoomControl={false}>
      ...
      {selectedPOI && (
        <div style={{ position: 'absolute', bottom: 90, left: '50%', transform: 'translateX(-50%)', zIndex: 1200, width: 290, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.18)', overflow: 'hidden', direction: 'rtl' }}>
          ...
          <div style={{ padding: '14px 16px 16px' }}>
            ...
            <button onClick={() => navigate(`/POIDetail?id=${selectedPOI.id}`)} style={{ width: '100%', padding: '11px', border: 'none', borderRadius: 13, background: 'linear-gradient(135deg, #0d9e6e, #0bba7e)', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>
              פרטים נוספים
            </button>
          </div>
        </div>
