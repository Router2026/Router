...
export default function AddReport() {
  ...
  const locationIdFromUrl = searchParams.get("location_id")
    ? parseInt(searchParams.get("location_id")!)
    : undefined;
  ...
  const canSubmit =
    formData.report_type !== "" && formData.content.trim().length > 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const finalLocationId = selectedPoi
        ? parseInt(selectedPoi.id)
        : locationIdFromUrl;
      ...
      await api.reports.create({
        ...
        reporter_name: isLoggedIn
          ? (user?.full_name ?? user?.username ?? formData.reporter_name)
          : formData.reporter_name || "אנונימי",
      });
      ...
      setTimeout(
        () =>
          navigate(
            finalLocationId ? `/POIDetail?id=${finalLocationId}` : "/Reports",
          ),
        1200,
      );
    } catch (err: unknown) {
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            transition: "all 0.2s ease",
            boxShadow: canSubmit
              ? "0 8px 20px rgba(13, 158, 110, 0.25)"
              : "none",
          }}
        >
