    ...
    try {
      await api.reports.create({
        ...
        reporter_name: isLoggedIn
          ? user?.username || formData.reporter_name
          : "אנונימי",
      });
      setSuccess(true);
      setTimeout(
        () => navigate(locationId ? `/POIDetail?id=${locationId}` : "/Reports"),
        1200,
      );
    } catch {
      setLoading(false);
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 16,
            background:
              formData.report_type && formData.content
                ? "linear-gradient(135deg, #0d9e6e, #0bba7e)"
                : "#e2e8f0",
            color:
              formData.report_type && formData.content ? "#fff" : "#94a3b8",
            fontSize: 16,
            fontWeight: 800,
            cursor:
              formData.report_type && formData.content
