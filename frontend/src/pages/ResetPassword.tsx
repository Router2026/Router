...
export default function ResetPassword() {
  ...
  const inputBase: React.CSSProperties = {
    ...
    direction: 'ltr', textAlign: 'left', outline: 'none', background: '#f8fafc',
  ...
  return (
    <div style={{ background: '#f0f4f3', minHeight: '100vh', direction: 'rtl' }}>
      <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '40px 24px 56px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)', marginTop: 6 }}>בחר סיסמה חדשה לחשבון שלך</div>
      </div>
      ...
      <div style={{ maxWidth: 420, margin: '-28px auto 0', padding: '0 20px 100px', position: 'relative', zIndex: 10 }}>
        <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 8px 40px rgba(0,0,0,0.1)', padding: '28px 24px 24px' }}>
          ...
          {error && (
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '12px 16px', color: '#dc2626', fontSize: 13, fontWeight: 600, marginBottom: 16, textAlign: 'center' }}>
              ⚠️ {error}
            </div>
          )}

          <button onClick={handleSubmit} disabled={loading}
            style={{ width: '100%', padding: '16px', border: 'none', borderRadius: 16, background: loading ? '#e2e8f0' : 'linear-gradient(135deg, #0f172a, #1e293b)', color: loading ? '#94a3b8' : '#fff', fontSize: 16, fontWeight: 900, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Heebo, sans-serif', transition: 'all 0.2s' }}>
