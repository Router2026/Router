...
export default function Login() {
  ...
  const toHebrewError = (e: { code?: string; message?: string }): string => {
    ...
    if (code === "AUTH_ERROR" || msg.includes("invalid email or password"))
      return "האימייל או הסיסמה שגויים. נסה שוב.";
    if (msg.includes("required")) return "אנא מלא אימייל וסיסמה";
    if (code === "DB_ERROR" || msg.includes("failed"))
      return "שגיאת שרת, נסה שוב עוד רגע";
    if (msg.includes("network") || msg.includes("fetch"))
      return "בעיית חיבור לאינטרנט, בדוק את הרשת שלך";
        ...
        >
          ...
          {unverifiedEmail && (
            ...
            >
              ...
              {resentVerification ? (
                <div style={{ marginTop: 6, color: "#0d9e6e" }}>
                  ✓ קישור אימות נשלח מחדש!
                </div>
              ) : (
                <button
