...
const HomeIcon = ({ a }: { a: boolean }) => <svg width="22" height="22" viewBox="0 0 24 24" fill={a ? '#0d9e6e' : 'none'} stroke={a ? '#0d9e6e' : '#94a3b8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>;
const MapIcon = ({ a }: { a: boolean }) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#0d9e6e' : '#94a3b8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6" /><line x1="8" y1="2" x2="8" y2="18" /><line x1="16" y1="6" x2="16" y2="22" /></svg>;
const RouteIcon = ({ a }: { a: boolean }) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#0d9e6e' : '#94a3b8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="5" cy="6" r="2" /><path d="M5 8v12" /><circle cx="19" cy="18" r="2" /><path d="M19 16V4" /><path d="M5 14h8a2 2 0 0 0 2-2V8" /><path d="M19 10h-8a2 2 0 0 1-2-2" /></svg>;
...
export default function Layout({ children, currentPageName }: { children: React.ReactNode; currentPageName: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100%' }}>
      ...
      {showNav && (
        <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 2000, background: '#fff', borderTop: '1px solid #e8f5f1', boxShadow: '0 -4px 20px rgba(13,158,110,0.08)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', maxWidth: 640, margin: '0 auto', padding: '6px 0 4px' }}>
            {NAV_ITEMS.map(({ label, value, Icon }) => {
              ...
              return (
                <button key={value} onClick={() => navigate(`/${value}`)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer', padding: '5px 10px', borderRadius: 12, minWidth: 44 }}>
          ...
          <div style={{ position: 'absolute', top: -44, left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none' }}>
            {isLoggedIn ? (
              ...
              <button onClick={() => navigate('/Register')} style={{ background: 'linear-gradient(135deg, #0d9e6e, #0bba7e)', border: 'none', borderRadius: 20, padding: '7px 18px', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'Heebo, sans-serif', boxShadow: '0 2px 12px rgba(13,158,110,0.25)', pointerEvents: 'all' }}>
                🚀 הרשמה / כניסה
              </button>
            )}
          </div>
