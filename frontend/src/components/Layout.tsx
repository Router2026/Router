...
const DiscoveryIcon = ({ a }: { a: boolean }) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? '#0d9e6e' : '#94a3b8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10" /><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" /></svg>;
...
const NAV_ITEMS = [
  { label: 'בית', value: 'Home', tourId: 'nav-home', Icon: ({ active }: { active: boolean }) => <HomeIcon a={active} />, path: '/' },
  { label: 'מפה', value: 'MapView', tourId: 'nav-map', Icon: ({ active }: { active: boolean }) => <MapIcon a={active} />, path: '/MapView' },
  { label: 'גילוי אתרים', value: 'Explore', tourId: 'nav-explore', Icon: ({ active }: { active: boolean }) => <DiscoveryIcon a={active} />, path: '/Explore' },
  { label: 'קהילה', value: 'trips', tourId: 'nav-community', Icon: ({ active }: { active: boolean }) => <CommunityIcon a={active} />, path: '/trips' },
  { label: 'דיווחים', value: 'Reports', tourId: 'nav-reports', Icon: ({ active }: { active: boolean }) => <AlertIcon a={active} />, path: '/Reports' },
  { label: 'פרופיל', value: 'Profile', tourId: 'nav-profile', Icon: ({ active }: { active: boolean }) => <PersonIcon a={active} />, path: '/Profile' },
];
