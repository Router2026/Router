...
// ── handleSubmit helpers (extracted to reduce cognitive complexity) ─────────────
...
function buildFileError(err: unknown): string {
  const { message, code } = toApiError(err);
  const msg: string = message ?? '';
  const errCode: string = code ?? '';
  if (errCode === 'LIMIT_REACHED') return 'הגעת למגבלת המדיה עבור מיקום זה';
  if (errCode === 'AUTH_ERROR' || msg.includes('401')) return 'פג תוקף ההתחברות — אנא רענן את הדף';
  if (msg.includes('413') || msg.toLowerCase().includes('too large') || msg.toLowerCase().includes('large'))
    return 'הקובץ גדול מדי — דחס את התמונה או בחר קובץ קטן יותר';
  if (msg.toLowerCase().includes('unsupported') || errCode === 'VALIDATION_ERROR')
    return 'סוג קובץ לא נתמך — בחר JPEG, PNG, WEBP, HEIC, MP4 או MOV';
  if (msg.toLowerCase().includes('failed to fetch') || msg.toLowerCase().includes('network'))
    return 'בעיית רשת — בדוק את החיבור לאינטרנט ונסה שוב';
  return msg || 'העלאה נכשלה — ודא שהקובץ תקין ונסה שוב';
}
...
async function submitFileUpload(
  ...
  caption: string,
): Promise<ReturnType<typeof api.locations.uploadMedia>> {
  if (!selectedFile) throw Object.assign(new Error('בחר קובץ'), { _earlyExit: true });
  return api.locations.uploadMedia(locationId, selectedFile, caption || undefined);
}
...
async function submitUrlUpload(
  ...
  caption: string,
): Promise<ReturnType<typeof api.locations.uploadMediaUrl>> {
  const url = urlInput.trim();
  if (!url) throw Object.assign(new Error('הכנס קישור'), { _earlyExit: true });
  if (!url.startsWith('http://') && !url.startsWith('https://'))
    throw Object.assign(new Error('הקישור חייב להתחיל ב-https://'), { _earlyExit: true });
  return api.locations.uploadMediaUrl(locationId, url, urlMediaType, caption || undefined);
}
...
export default function UploadPhotoButton({ locationId, onUploaded }: Props) {
  ...
  const openModal = () => {
    ...
    // Use requestAnimationFrame so the dialog is in the DOM before calling showModal
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = mode === 'file'
        ? await submitFileUpload(locationId, selectedFile, caption)
        : await submitUrlUpload(locationId, urlInput, urlMediaType, caption);
      setXpResult(res.xp);
      closeModal();
      onUploaded?.(res.media);
    } catch (err: unknown) {
      if (toApiError(err)._earlyExit) {
        setError((err as Error).message);
      } else {
        setError(buildFileError(err));
      }
    } finally {
      setUploading(false);
    }
  ...
  const captionInputId = 'upload-photo-caption';
  const urlInputId = 'upload-photo-url';
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '28px 24px', width: '100%', maxWidth: 440, direction: 'rtl', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          {mode === 'file' ? (
            ...
            <>
              ...
              <label htmlFor={urlInputId} style={{ fontSize: 13, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 6 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            <label htmlFor={captionInputId} style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>
              כיתוב (אופציונלי)
            </label>
            <input
              id={captionInputId}
