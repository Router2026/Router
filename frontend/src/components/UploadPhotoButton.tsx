...
function FileSection({ preview, isVideoPreview, fileRef, handleFileChange, onClear }: Readonly<{
  preview: string | null;
    ...
    <>
      ...
      {preview ? (
        <div style={{ position: 'relative', marginBottom: 12 }}>
          {isVideoPreview ? (
            <video src={preview} controls style={{ width: '100%', maxHeight: 200, borderRadius: 12, border: '2px solid #e2e8f0', background: '#000' }} playsInline>
              <track kind="captions" />
            </video>
          ) : (
          ...
          <button onClick={onClear} style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.5)', border: 'none', borderRadius: '50%', width: 28, height: 28, cursor: 'pointer', color: '#fff', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
        </div>
      ) : (
        <button onClick={() => fileRef.current?.click()} style={{ width: '100%', height: 120, border: '2px dashed #cbd5e1', borderRadius: 12, background: '#f8fafc', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, color: '#64748b', marginBottom: 12 }}>
...
function UrlSection({ urlInput, urlMediaType, onKeyDown, setUrlInput, setUrlMediaType }: Readonly<{
  urlInput: string;
  urlMediaType: 'image' | 'video';
  onKeyDown: (e: React.KeyboardEvent) => void;
  setUrlInput: (v: string) => void;
  setUrlMediaType: (v: 'image' | 'video') => void;
}>) {
  return (
    <>
...
export default function UploadPhotoButton({ locationId, onUploaded }: Readonly<Props>) {
  const { user } = useAuth();
  ...
  const openModal = () => {
    setShowModal(true); setError(null); setUrlInput('');
    setSelectedFile(null); setPreview(null); setCaption(''); setIsVideoPreview(false);
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedFile(f); setError(null); setIsVideoPreview(isVideo);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = e?.code === 'LIMIT_REACHED'
  ...
  const canSubmit = mode === 'file' ? !!selectedFile : !!urlInput.trim();

  return (
    <>
      {xpResult && <XpToast xp={xpResult} onDone={() => setXpResult(null)} />}

      <button onClick={openModal} style={{ width: '100%', padding: '16px', border: '2px dashed #0d9e6e', borderRadius: 18, background: '#f0fdf8', color: '#0d9e6e', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'Heebo, sans-serif', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <button type="button" aria-label="סגור" onClick={closeModal}
            style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', border: 'none', cursor: 'default', padding: 0 }} />
          <dialog open aria-labelledby="upload-dialog-title"
            style={{ position: 'relative', zIndex: 1, background: '#fff', borderRadius: 20, padding: '28px 24px', width: '100%', maxWidth: 440, direction: 'rtl', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto', border: 'none', margin: 0 }}>

            <div id="upload-dialog-title" style={{ fontSize: 20, fontWeight: 900, color: '#1a2e2a', marginBottom: 6 }}>📷 העלה תמונה / סרטון</div>
            ...
            {mode === 'file' ? (
              <FileSection
                preview={preview} isVideoPreview={isVideoPreview} fileRef={fileRef}
                handleFileChange={handleFileChange}
                onClear={() => { setSelectedFile(null); setPreview(null); setIsVideoPreview(false); if (fileRef.current) fileRef.current.value = ''; }}
              />
            ) : (
              <UrlSection
                urlInput={urlInput} urlMediaType={urlMediaType} onKeyDown={onKeyDown}
                setUrlInput={setUrlInput} setUrlMediaType={setUrlMediaType}
              />
            )}

            {/* Caption */}
            <div style={{ marginBottom: 8 }}>
              <label htmlFor="upload-caption" style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>כיתוב (אופציונלי)</label>
              <input id="upload-caption" type="text" value={caption} onChange={e => setCaption(e.target.value)}
                placeholder="תיאור קצר של התמונה / הסרטון..." maxLength={200}
            ...
            {error && <div style={{ marginTop: 4, marginBottom: 8, color: '#ef4444', fontSize: 13, fontWeight: 600 }}>⚠️ {error}</div>}
