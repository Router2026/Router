  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'file') {
        ...
        // BUG FIX: api.locations.uploadMedia internally calls fileToBase64 (FileReader).
        // On mobile, FileReader can silently fail for HEIC files or when the device
        // is under memory pressure. Wrap with a descriptive catch so the user gets a
        // real error message instead of "Failed to fetch".
        //
        // Also: pass locationId directly as-is (string | number) — casting to Number()
        // produces NaN for UUID-based IDs, which makes the URL /locations/NaN/media.
      ...
      // Map common error codes / messages to Hebrew user-friendly text
      const msg: string = err?.message ?? '';
      if (err?.code === 'LIMIT_REACHED') {
        setError('הגעת למגבלת המדיה עבור מיקום זה');
      } else if (err?.code === 'AUTH_ERROR' || msg.includes('401')) {
        setError('פג תוקף ההתחברות — אנא רענן את הדף');
      } else if (msg.includes('413') || msg.toLowerCase().includes('too large') || msg.toLowerCase().includes('large')) {
        setError('הקובץ גדול מדי — דחס את התמונה או בחר קובץ קטן יותר');
      } else if (msg.toLowerCase().includes('unsupported') || err?.code === 'VALIDATION_ERROR') {
        setError('סוג קובץ לא נתמך — בחר JPEG, PNG, WEBP, HEIC, MP4 או MOV');
      } else if (msg.toLowerCase().includes('failed to fetch') || msg.toLowerCase().includes('network')) {
        setError('בעיית רשת — בדוק את החיבור לאינטרנט ונסה שוב');
      } else {
        setError(msg || 'העלאה נכשלה — ודא שהקובץ תקין ונסה שוב');
      }
    } finally {
      setUploading(false);
    }
